...
function sendPost(data) {
  ...
  axios.post("/v1/post", {
    ...
    const end = parseInt(transactionNumber.innerHTML) + res.data?.data?.txIds?.length
    const dur = 2000
    console.log(res.data?.data?.txIds);
    animateValue(transactionNumber, start, end, dur)
    transactionNumber.innerHTML = parseInt(transactionNumber.innerHTML) + res.data?.data?.txIds?.length
...
var fieldDistance, energyBar, replayMessage, fieldLevel, levelCircle, transactionNumber
...
function init(event) {
  ...
  transactionNumber = document.getElementById('transaction')
